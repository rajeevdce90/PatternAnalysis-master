...
# Try to import blueprints from app module if available
# Commented out due to null byte error
# try:
...
# Register advanced data handling blueprint if available - disabled due to null byte error
# if has_app_module:
...
def upload_file():
    try:
        ...
        # Convert DataFrame rows to list of dictionaries for bulk indexing
        ...
        logger.info(f"Bulk indexing {len(records)} records to {target_index}")
        try:
            # Get the client connection
            client = opensearch_client.get_client()
            
            # Check if index exists, create if not
            index_exists = client.indices.exists(index=target_index)
            logger.info(f"Index {target_index} exists: {index_exists}")
            ...
            bulk_response = bulk_index(target_index, records)
            logger.info(f"Bulk indexing complete: {bulk_response}")
            
            # Log success/failure count
            success_items = [item for item in bulk_response.get('items', []) 
                         if item.get('index', {}).get('status') in [200, 201]]
            success_count = len(success_items)
            
            logger.info(f"Successfully indexed {success_count} out of {len(records)} records to {target_index}")
            
            # Log any errors
            error_items = [item for item in bulk_response.get('items', []) 
                         if item.get('index', {}).get('status') not in [200, 201]]
            if error_items:
                logger.warning(f"Indexing errors: {error_items[:5]}")
